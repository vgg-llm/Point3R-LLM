#!/bin/bash
#SBATCH --job-name="RoboFAC Training and Evaluation"
#SBATCH -o sbatch_log/robofac.%j.out
#SBATCH --partition=cms_cvlab
#SBATCH --gres=gpu:8
#SBATCH --nodes=1

# Experiment: RoboFAC (Robotic Failure Analysis and Correction)
# Video-only — no pointer memory / preprocessed input needed.

# source venv/bin/activate  # using conda env instead

export EXP_NAME="robofac_Qwen3VL_4b"
export MODEL_PATH="Qwen/Qwen3-VL-4B-Instruct"
export DATASETS="robofac"

# No pointer memory for video-only tasks
