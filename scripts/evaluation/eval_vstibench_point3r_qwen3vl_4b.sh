#!/bin/bash
# VSTIBench Point3R Evaluation Script for Qwen3-VL-4B
...
benchmarks="vstibench_point3r"
output_path=logs/vstibench_4b_$(TZ="Asia/Shanghai" date "+%Y%m%d_%H%M%S")
model_path=./outputs/vstibench_Qwen3VL_4b_memfeat
...
echo "============================================="
echo "Evaluating VSTIBench Point3R (4B model)"
...
accelerate launch --num_processes=8 --main_process_port 29501 -m lmms_eval \
    ...
    --tasks ${benchmarks} \
    --batch_size 1 \
    --log_samples_suffix vstibench_eval \
    --log_samples \
