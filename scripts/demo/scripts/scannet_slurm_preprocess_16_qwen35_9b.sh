...
for task_id in $(seq 0 $((TOTAL_TASKS - 1))); do
    ...
    sbatch --job-name=scannet_${task_id}_preprocessing_for_Qwen3.5-Point3R-9B \
           --output=logs/scannet_chunk_${task_id}_${SAMPLE_CT}_frame_qwen35_9b.log \
           --error=logs/scannet_chunk_${task_id}_${SAMPLE_CT}_frame_qwen35_9b.log \
           --partition=cms_cvlab \
           --gres=gpu:1 \
           --nodes=1 \
           scripts/demo/slurm_preprocess_scannet_task_qwen35_9b.sh ${task_id} ${TOTAL_TASKS} ${SAVE_PATH} ${SAMPLE_CT}
done
